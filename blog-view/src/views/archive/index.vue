...
<div>
    ...
    <div class="ui attached segment">
        <div class="timeline">
            <div :class="colorObj[index%5]" v-for="(value,index) in archiveMap" :key="`archiveMap-${index}`">
                <div class="tl-header">
                    <a class="ui large label m-text-500">{{ value.title }}</a>
                </div>
                <div class="tl-item" v-for="(item,i) in value.list" :key="`archive${i}`">
                    <div class="tl-wrap">
                        <span class=" tl-date "> {{item.createTime | formatDay }} 日 </span>
                        <router-link :to="`/blog/${item.blogId}`" v-if="item.type === 1">
                            <div class="ui left pointing label tl-title">{{ item.title }}</div>
                        </router-link>
