            ...
            <div class="ui link three stackable cards">
                <a :href="item.url" target="_blank" class="card" :class="randomColor()" v-for="(item,index) in randomFriendList" :key="index">
                    ...
                    <div class="content">
                        ...
                        <div class="description" style="color: white">{{ item.introduction }}</div>
                    </div>
                </a>
            </div>
        </div>
...
export default {
    ...
    data() {
        return {
            ...
            friendList: [],
            message: {},
            colorObj: ['bg-1', 'bg-2', 'bg-3', 'bg-4', 'bg-5', 'bg-6 ', 'bg-7', 'bg-8', 'bg-9'],
        }
    },
    components: {
        Title,
    ...
    methods: {
        ...
        // // 标签背景颜色随机
        // randomColor() {
        //   let index = Math.floor(Math.random() * this.colorObj.length);
        //   
        //   return this.colorObj[index];
        // },
